...
for (const file of filesToUpdate) {
    ...
    // Find the ElementsKit wrapper widget
    let ekitWidget = $('.elementor-widget-ekit-nav-menu');
    if (ekitWidget.length > 0) {
        let ekitNav = ekitWidget.find('.ekit_menu_responsive_mobile');
        if (ekitNav.length > 0) {
        ...
        // Move the ENTIRE widget into .custom-header-actions
        let actionsContainer = $('.custom-header-actions');
        if (actionsContainer.length > 0) {
            // If the widget is not already inside the actions container, move it
            if (actionsContainer.find('.elementor-widget-ekit-nav-menu').length === 0) {
                let widgetHtml = $.html(ekitWidget);
                actionsContainer.append(widgetHtml);
                ekitWidget.remove();
            }
        }
